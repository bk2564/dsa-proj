import { ShowHashMap, ShowArray } from './show-alg.js'
...
export function HashMapExecutionSection({ title, subtitle, steps, getHighlight }) {
  return (
    <section>
      <h2 className="text-xl font-semibold mb-3">{title}</h2>
      <code className="text-sm text-gray-400 mb-4">{subtitle}</code>
...
function HashMapStepCard({ stepNumber, step, highlight }) {
  return (
    <div className="bg-gray-900 rounded-xl p-4">
      <p className="text-sm mb-3 text-gray-300">
        Step {stepNumber}: {step.text}
